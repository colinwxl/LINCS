...
paginationUtils = {
    ...
    query: function query(itemCollection, options) {
        if (_.isNumber(options.limit)) {
            itemCollection.query('limit', options.limit).query('offset', options.limit * (options.page - 1));
        }
    },
...
pagination = function pagination(bookshelf) {
    ...
    _.extend(bookshelf.Model.prototype, {
        ...
        fetchPage: function fetchPage(options) {
            ...
            return _promise2.default.join(collectionPromise, countPromise).then(function formatResponse(results) {
